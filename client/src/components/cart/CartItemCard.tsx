    ...
    return <article className="grid gap-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-[0_18px_60px_-38px_rgba(15,23,42,0.35)] sm:grid-cols-[132px_minmax(0,1fr)_auto] sm:p-5">
        ...
        <div className="min-w-0"><p className="text-xs font-black uppercase tracking-wider text-blue-600">{product.brand}</p><h3 className="mt-1 text-lg font-black text-slate-950 sm:text-xl">{product.name}</h3><p className="mt-2 text-sm text-slate-500">{product.stock} unidades disponibles</p>
            <div className="mt-4 inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 p-1" aria-label={`Cantidad de ${product.name}`}>
                <button type="button" aria-label="Disminuir cantidad" disabled={isMutating || item.quantity <= 1} onClick={() => onUpdateQuantity(product._id, item.quantity - 1)} className="flex h-11 w-11 items-center justify-center rounded-lg text-slate-700 transition hover:bg-white hover:text-blue-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 disabled:cursor-not-allowed disabled:opacity-35"><FiMinus /></button><span className="min-w-10 text-center font-black" aria-live="polite">{item.quantity}</span><button type="button" aria-label="Aumentar cantidad" disabled={isMutating || item.quantity >= product.stock} onClick={() => onUpdateQuantity(product._id, item.quantity + 1)} className="flex h-11 w-11 items-center justify-center rounded-lg text-slate-700 transition hover:bg-white hover:text-blue-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 disabled:cursor-not-allowed disabled:opacity-35"><FiPlus /></button>
            </div>
        </div>
        <div className="flex items-end justify-between gap-4 sm:flex-col sm:items-end"><div className="sm:text-right"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Subtotal</p><p className="mt-1 text-xl font-black text-slate-950">${(price * item.quantity).toFixed(2)}</p></div><button type="button" aria-label={`Eliminar ${product.name} del carrito`} disabled={isMutating} onClick={() => onRemove(product._id)} className="inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-bold text-red-600 transition hover:bg-red-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-600 disabled:cursor-not-allowed disabled:opacity-50"><FiTrash2 /> <span className="hidden sm:inline">Eliminar</span></button></div>
    </article>;
};
